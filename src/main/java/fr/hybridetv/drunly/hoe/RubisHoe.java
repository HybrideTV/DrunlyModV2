package fr.hybridetv.drunly.hoe;
...
public class RubisHoe extends ItemHoe {
	
	private final float speed;

	public RubisHoe(ToolMaterial material, String name) {
		super(material);
        ...
        else
        {
            ...
            if (facing != EnumFacing.DOWN && worldIn.isAirBlock(pos.up()))
            {
                if (block == Blocks.GRASS || block == Blocks.GRASS_PATH)
                {
                	...
                	this.setBlock(itemstack, player, worldIn, pos.add(X, Y - 1, Z), Blocks.FARMLAND.getDefaultState());
                    this.setBlock(itemstack, player, worldIn, pos.add(X - 1, Y - 1, Z - 1), Blocks.FARMLAND.getDefaultState());
                    this.setBlock(itemstack, player, worldIn, pos.add(X + 1, Y - 1, Z + 1), Blocks.FARMLAND.getDefaultState());
                    this.setBlock(itemstack, player, worldIn, pos.add(X, Y - 1, Z + 1), Blocks.FARMLAND.getDefaultState());
                    this.setBlock(itemstack, player, worldIn, pos.add(X + 1, Y - 1, Z), Blocks.FARMLAND.getDefaultState());
                    this.setBlock(itemstack, player, worldIn, pos.add(X - 1, Y - 1, Z), Blocks.FARMLAND.getDefaultState());
                    this.setBlock(itemstack, player, worldIn, pos.add(X, Y - 1, Z - 1), Blocks.FARMLAND.getDefaultState());
                    this.setBlock(itemstack, player, worldIn, pos.add(X + 1, Y - 1, Z - 1), Blocks.FARMLAND.getDefaultState());
                    this.setBlock(itemstack, player, worldIn, pos.add(X - 1, Y - 1, Z + 1), Blocks.FARMLAND.getDefaultState());
