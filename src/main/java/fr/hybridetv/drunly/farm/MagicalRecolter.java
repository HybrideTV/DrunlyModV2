...
public class MagicalRecolter extends ItemHoe {
	
	private final float speed;

	public MagicalRecolter (ToolMaterial toolrubis, String name) {
		super(toolrubis);
        this.speed = toolrubis.getAttackDamage() + 1.0F;
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float X, float Y, float Z)
    {
        ...
        else
        {
            ...
            {
                if (block == Blocks.POTATOES || block == Blocks.CARROTS || block == BlocksMod.erablecrop || block == BlocksMod.fraisecrop || block == BlocksMod.kiwicrop || block == BlocksMod.treflecrop )
                {
                	this.destroyAndDropBlock(itemstack, player, worldIn, pos, iblockstate);
                	this.destroyAndDropBlock(itemstack, player, worldIn, pos.add(X, Y , Z), Blocks.AIR.getDefaultState());
                    this.destroyAndDropBlock(itemstack, player, worldIn, pos.add(X - 1, Y , Z - 1), Blocks.AIR.getDefaultState());
                    this.destroyAndDropBlock(itemstack, player, worldIn, pos.add(X + 1, Y , Z + 1), Blocks.AIR.getDefaultState());
                    this.destroyAndDropBlock(itemstack, player, worldIn, pos.add(X, Y , Z + 1), Blocks.AIR.getDefaultState());
                    this.destroyAndDropBlock(itemstack, player, worldIn, pos.add(X + 1, Y , Z), Blocks.AIR.getDefaultState());
                    this.destroyAndDropBlock(itemstack, player, worldIn, pos.add(X - 1, Y , Z), Blocks.AIR.getDefaultState());
                    this.destroyAndDropBlock(itemstack, player, worldIn, pos.add(X, Y , Z - 1), Blocks.AIR.getDefaultState());
                    this.destroyAndDropBlock(itemstack, player, worldIn, pos.add(X + 1, Y , Z - 1), Blocks.AIR.getDefaultState());
                    this.destroyAndDropBlock(itemstack, player, worldIn, pos.add(X - 1, Y , Z + 1), Blocks.AIR.getDefaultState());
    ...
    private void destroyAndDropBlock(ItemStack itemstack, EntityPlayer player, World worldIn, BlockPos pos,
